...
<body class="bg-gray-900 text-gray-200 flex justify-center items-center min-h-screen">
    <div class="bg-gray-800 p-6 rounded-lg shadow-md w-full max-w-md">
        ...
        @session('error')
          <p class="text-center bg-red-600 p-3 text-white mb-4">{{session("error")}}</p>
        @endsession
        @if ($errors->any())
            @foreach ($errors->all() as $error)
                <p class="text-center bg-red-600 p-3 rounded text-white mb-4">{{ $error }}</p>
            @endforeach
        @endif
        ...
        otpInputs.forEach((input, index) => {
            input.addEventListener('input', () => {
                ...
                if (Array.from(otpInputs).every(i => i.value.length === 1)) {
                    document.querySelector('input[name="otp"]').value = Array.from(otpInputs).map(i => i.value).join('');
